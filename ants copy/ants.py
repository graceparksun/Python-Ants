...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        """
        if insect.is_ant: # if insect is ant
            if self.ant is None: # if there is no ant
                self.ant = insect 
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while (count < self.min_range) and throw_place != hive:
            throw_place = throw_place.entrance
            count += 1
        while (len(throw_place.bees) == 0) and throw_place != hive:
            throw_place = throw_place.entrance
            count += 1
        if throw_place == hive or count > self.max_range:
            return None
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            for b in bees_in_place:
                b.armor -= self.damage
            self.place.bees = [b for b in bees_in_place if b.armor != 0]
            self.place.remove_insect(self)  
        # END Problem 4A
...
# BEGIN Problem 5A
class WallAnt(Ant):
    """A WallAnt doesn't do anything but has a large armor value"""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is more costly and watersafe."""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.is_Queen:
            ...
            while (queen_place != None):
                if queen_place.ant:
                    if not(queen_place.ant in self.ant_doubled):
                        queen_place.ant.damage *= 2
                        self.ant_doubled.append(queen_place.ant)
                    if queen_place.ant.container and queen_place.ant.ant and not(queen_place.ant.ant in self.ant_doubled):
                        queen_place.ant.ant.damage *= 2
                        self.ant_doubled.append(queen_place.ant.ant)
